...
class IdentifierRepositoryTest {
    ...
    init {
        ...
        every { identifierDao.insert(identifier) } returns Unit
        ...
        every {
            runBlocking {
                val resolveIdentifierNetworkOperation = spyk(ResolveIdentifierNetworkOperation(identifierRepository.apiProvider, "testUrl", identifier.id))
                resolveIdentifierNetworkOperation.fire()
            }
        } returns Result.Success(expectedIdentifierDocument)
    }
