...
class CardRepository @Inject constructor(database: SdkDatabase, private val picNetworkOperation: PicNetworkOperation) {
    ...
    suspend fun getRequest(url: String) = picNetworkOperation.getRequestToken(url)

    suspend fun sendIssuanceResponse(url: String, serializedResponse: String)= picNetworkOperation.sendIssuanceResponse(url, serializedResponse)
