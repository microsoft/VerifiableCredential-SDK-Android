...
class JwtDomainLinkageCredentialValidatorTest {
    ...
    fun `validate well known config document with correct inputs`() {
        ...
        runBlocking {
            val validated = jwtDomainLinkageCredentialValidator.validate(domainLinkageCredentialJwt, validRpDid, "www.google.com")
    ...
    fun `failing validation of well known config document with incorrect issuer DID`() {
        ...
        coEvery { mockedJwtValidator.verifySignature(any()) } returns true
        runBlocking {
            val validated = jwtDomainLinkageCredentialValidator.validate(domainLinkageCredentialJwt, invalidRpDid, "www.google.com")
    ...
    fun `failing validation of well known config document with incorrect domain url`() {
        ...
        coEvery { mockedJwtValidator.verifySignature(any()) } returns true
        runBlocking {
            val validated = jwtDomainLinkageCredentialValidator.validate(domainLinkageCredentialJwt, invalidRpDid, "www.test.com")
            Assertions.assertThat(validated).isFalse()
        }
    }
