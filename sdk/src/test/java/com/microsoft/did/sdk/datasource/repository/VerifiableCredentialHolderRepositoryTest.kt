...
class VerifiableCredentialHolderRepositoryTest {
    ...
    private val mockedRequestedPresentationSubmissionMap: RequestedVchPresentationSubmissionMap = mockk()
    private val mockedPrimeIdentifier: Identifier = mockk()
    private val mockedPairwiseIdentifier: Identifier = mockk()
    private val mockedIssuanceResponseFormatter: IssuanceResponseFormatter = mockk()
    private val mockedPresentationResponseFormatter: PresentationResponseFormatter = mockk()
    private val mockedExchangeResponseFormatter: ExchangeResponseFormatter = mockk()
    private val mockedRevocationResponseFormatter: RevocationResponseFormatter = mockk()
    ...
    init {
        ...
        verifiableCredentialHolderRepository = VerifiableCredentialHolderRepository(
            database,
            apiProvider,
            mockedIssuanceResponseFormatter,
            mockedPresentationResponseFormatter,
            mockedExchangeResponseFormatter,
            mockedRevocationResponseFormatter,
            serializer
        )
    ...
    fun `send issuance response successfully`() {
        ...
        runBlocking {
            val actualResult = verifiableCredentialHolderRepository.sendIssuanceResponse(mockedIssuanceResponse, mockedRequestedVchMap)
...
//        val expectedVch: LiveData<VerifiableCredentialHolder> = MutableLiveData()
    ...
    fun `revoke verifiable presentation failed response`() {
        val revokeException = RevocationException("Unable to revoke VP")
        coEvery { anyConstructed<SendVerifiablePresentationRevocationRequestNetworkOperation>().fire() } returns Result.Failure(
            revokeException
        )
    ...
    private fun setUpFormatter() {
        every { mockedIssuanceResponseFormatter.formatResponse(any(), any(), any()) } returns expectedSignedResponseToken
        every { mockedPresentationResponseFormatter.formatResponse(any(), any(), any()) } returns expectedSignedResponseToken
        every { mockedExchangeResponseFormatter.formatResponse(any(), any()) } returns expectedSignedResponseToken
    }
    ...
    private fun setUpIssuanceResponse() {
        ...
        every { mockedIssuanceResponse.requestedIdTokenMap } returns expectedIdTokenContextMapping
        every { mockedIssuanceResponse.requestedSelfAttestedClaimMap } returns expectedSelfAttestedClaimContext
    }

    private fun setUpPresentationResponse() {
        val oidcRequestContent: PresentationRequestContent = mockk()
